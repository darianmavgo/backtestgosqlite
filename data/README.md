...
- `settings.db`: Master configuration and symbol lists (`leveraged_etf`, `momentum_candidates`, `backtested_win_20_10d`), the ETF universes (`etf_universe`: lists `all`, `6yr`, `sweep`) and per-ETF decision-tree configs (`etf_dt_strategies`). No txt/CSV ticker lists are used.
