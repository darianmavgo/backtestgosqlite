-- QQQ closed up from the prior close AND traded more volume than the prior
...
WITH diffs AS (
    ...
    FROM backtest_start
    WHERE symbol = 'QQQ' AND length(Date) = 10
)
INSERT INTO sig_qqq_up1_buy_sqqq_dates_slice (date, up_and_vol_up)
SELECT
    date,
    1 AS up_and_vol_up
FROM diffs
