...
func Main() {
	...
	conf := DefaultConfig()
	d := conf
	flag.StringVar(&conf.Db, "db", d.Db, "Path to shared-account SQLite results database (defaults to latest in reports/)")
	flag.Parse()
	conf.Args = flag.Args()
...
func Run(conf Config) error {
	...
	if targetDB == "" {
		...
		if err != nil || len(files) == 0 {
			return fmt.Errorf("No shared account database found in reports/. Run backtest first.")
		}
	...
	if err != nil {
		return fmt.Errorf("Failed to open database %s: %v", targetDB, err)
	}
	defer db.Close()
