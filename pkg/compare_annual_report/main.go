...
type Config struct {
	SharedDb     string   // -shared-db
	StandaloneDb string   // -standalone-db
...
func DefaultConfig() Config {
	return Config{
		SharedDb:     appenv.ReportFile("shared_sig-voo-buy-tecl_bb-capitulation_2.db"),
		StandaloneDb: appenv.ReportFile("sig-voo-buy-tecl_4.db"),
		MarketDb:     appenv.MarketDB(),
		Html:         appenv.ReportFile("annual_comparison_standalone_vs_shared.html"),
	}
}
...
func Main() {
	...
	conf := DefaultConfig()
	d := conf
	flag.StringVar(&conf.SharedDb, "shared-db", d.SharedDb, "Path to shared account SQLite database")
	flag.StringVar(&conf.StandaloneDb, "standalone-db", d.StandaloneDb, "Path to standalone sig-voo-buy-tecl SQLite database")
	flag.StringVar(&conf.MarketDb, "market-db", d.MarketDb, "Path to market history SQLite database")
	flag.StringVar(&conf.Html, "html", d.Html, "Path to export HTML comparison report")
	flag.Parse()
	conf.Args = flag.Args()
...
func Run(conf Config) error {
	...
	db, err := sqlx.Open("sqlite", conf.SharedDb+"?_pragma=journal_mode(WAL)&_pragma=busy_timeout(15000)")
	if err != nil {
		return fmt.Errorf("Failed to open shared DB: %v", err)
	}
	defer db.Close()

	// Attach standalone and market databases
	attachStandalone := fmt.Sprintf("ATTACH DATABASE '%s' AS standalone;", conf.StandaloneDb)
	if _, err := db.Exec(attachStandalone); err != nil {
		return fmt.Errorf("Failed to attach standalone DB %s: %v", conf.StandaloneDb, err)
	}

	attachMarket := fmt.Sprintf("ATTACH DATABASE '%s' AS market;", conf.MarketDb)
	if _, err := db.Exec(attachMarket); err != nil {
		return fmt.Errorf("Failed to attach market DB %s: %v", conf.MarketDb, err)
	}
	...
	if err := db.Select(&rows, query); err != nil {
		return fmt.Errorf("Failed to execute comparison query: %v", err)
	}
	...
	if conf.Html != "" {
		err := generateHTMLReport(conf.Html, rows, sharedCurve, standaloneCurve, preempted, standaloneInitialCapital, sharedInitialCapital)
		if err != nil {
