...
func Main() {
	...
	flag.StringVar(&conf.SymbolsFrom, "symbols-from", d.SymbolsFrom, "Study report DB (e.g. reports/voo_up3_etf.db) whose etf_compare view supplies the trade symbols, ranked by rank_cagr (use with -top-cagr)")
	...
	flag.StringVar(&conf.Html, "html", d.Html, "Path to export HTML comparison report (single-strategy mode; defaults to reports/<strategy>_gridsearch.html)")
