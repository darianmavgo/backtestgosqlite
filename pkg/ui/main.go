...
func Main() {
	conf := DefaultConfig()
	d := conf
	flag.StringVar(&conf.Port, "port", d.Port, "Port to bind UI server")
	flag.Parse()
	conf.Args = flag.Args()
...
// Run serves the review UI on conf.Port until the listener fails.
func Run(conf Config) error {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(indexHTML) > 0 {
			...
			content, _ := os.ReadFile(filepath.Join(wcDir, "pkg", "ui", "web", "index.html"))
			w.Write(content)
		}
	})

	mux.HandleFunc("/api/summary", handleSummary)
	mux.HandleFunc("/api/symbol", handleSymbolDetails)
	mux.HandleFunc("/api/databases", handleDatabases)
	mux.HandleFunc("/api/query", handleQuery)
	mux.HandleFunc("/api/backtest/run", handleTriggerBacktest)
	mux.HandleFunc("/api/backtest/status", handleTaskStatus)
