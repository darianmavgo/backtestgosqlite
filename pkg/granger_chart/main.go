...
// Config holds the settings of a Granger chart report.
type Config struct {
	DB      string // study results DB holding granger_causality
...
func DefaultConfig() Config {
	return Config{
		DB:      appenv.ReportFile("march_april_voo_gld_uten.db"),
...
func Main() {
	...
	log.Printf("Successfully generated Go-Echarts tutorial visualization at %s", out)
}

// Run renders the Granger causality dashboard and returns the written path.
func Run(conf Config) (string, error) {
	dbPath := conf.DB
