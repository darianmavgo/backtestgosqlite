...
type DailyPoint struct {
	...
	AgncEquity      float64 `json:"agnc_equity"`
	AgncDD          float64 `json:"agnc_dd"`
	AgncShares      int     `json:"agnc_shares"`
	UproShares      int     `json:"upro_shares"`
	Cash            float64 `json:"cash"`
	CumulativeDivs  float64 `json:"cumulative_divs"`
...
func main() {
	...
	htmlOutput := flag.String("html", "reports/rank6_agnc_sweep_vs_voo.html", "Path to export interactive HTML comparison chart")
	flag.Parse()
	...
	// 1. Fetch Bars for VOO, UPRO, and AGNC
	var vooBars, uproBars, agncBars []BarData
	...
	_ = db.Select(&agncBars, `SELECT substr(Date, 1, 10) AS Date, open, high, low, close, "Adj Close", volume FROM backtest_start WHERE symbol = 'AGNC' ORDER BY substr(Date, 1, 10) ASC;`)

	// 2. Fetch Dividends for AGNC
	var divList []DividendEvent
	_ = db.Select(&divList, `SELECT symbol, ex_date, amount FROM corporate_dividends WHERE symbol = 'AGNC' ORDER BY ex_date ASC;`)

	agncDivMap := make(map[string]float64)
	for _, d := range divList {
		agncDivMap[d.ExDate] = d.Amount
	}

	// Align dates across all 3 assets
	uproMap := make(map[string]BarData)
	for _, b := range uproBars {
		uproMap[b.Date] = b
	}
	agncMap := make(map[string]BarData)
	for _, b := range agncBars {
		agncMap[b.Date] = b
	}

	// Filter common timeline
	var commonDates []string
	for _, b := range vooBars {
		if _, ok1 := uproMap[b.Date]; ok1 {
			if _, ok2 := agncMap[b.Date]; ok2 {
	...
	// Start with $100k invested 100% in AGNC shares on Day 0
	startAgncPrice := agncMap[commonDates[0]].Close
	agncShares := int(*initialCapital / startAgncPrice)
	cash := *initialCapital - float64(agncShares)*startAgncPrice
	uproShares := 0
	uproEntryPrice := 0.0

	// Benchmark holdings
	vooShares := *initialCapital / vooDateMap[commonDates[0]].Close
	agncOnlyShares := int(*initialCapital / startAgncPrice)
	agncOnlyCash := *initialCapital - float64(agncOnlyShares)*startAgncPrice
	agncOnlyDivs := 0.0
	...
	for i := 0; i < len(commonDates); i++ {
		...
		agncClose := agncMap[date].Close

		// 1. Process Ex-Dividend Payments for AGNC
		if divAmt, hasDiv := agncDivMap[date]; hasDiv {
			if agncShares > 0 {
				divPayout := float64(agncShares) * divAmt
				cash += divPayout
				cumulativeDivs += divPayout
			}
			if agncOnlyShares > 0 {
				payout := float64(agncOnlyShares) * divAmt
				agncOnlyCash += payout
				agncOnlyDivs += payout
			}
		}
		...
		// 2. Check 3-Day Dip Signal Entry into UPRO (65% Allocation)
		if signalMap[date] && i > inUproUntil && !inUpro {
			totalPortVal := cash + float64(agncShares)*agncClose
			targetUproCap := totalPortVal * *allocRatio

			// Liquidate AGNC to fund UPRO purchase
			neededFromAgnc := targetUproCap - cash
			if neededFromAgnc > 0 && agncShares > 0 {
				sharesToSell := int(math.Ceil(neededFromAgnc / agncClose))
				if sharesToSell > agncShares {
					sharesToSell = agncShares
				}
				cash += float64(sharesToSell) * agncClose
				agncShares -= sharesToSell
			}
			...
			if shares > 0 {
				...
				// Find Exit Point (+5% TP or 8 Days)
		...
		// 3. Check Trade Exit: Liquidate UPRO and SWEEP PROCEEDS BACK INTO AGNC!
		if inUpro && i == inUproUntil {
			...
			// Reinvest all available cash back into AGNC shares to resume max dividend yield
			newAgncShares := int(cash / agncClose)
			if newAgncShares > 0 {
				agncShares += newAgncShares
				cash -= float64(newAgncShares) * agncClose
			}
		...
		// 4. Calculate Portfolio Equities & Drawdowns
		strategyEquity := cash + float64(agncShares)*agncClose + float64(uproShares)*uproClose
		vooEquity := vooShares * vooClose
		agncOnlyEquity := agncOnlyCash + float64(agncOnlyShares)*agncClose
		...
		if agncOnlyEquity > agncPeak {
			agncPeak = agncOnlyEquity
		}
		agncDD := (agncPeak - agncOnlyEquity) / agncPeak * 100.0
		if agncDD > agncMaxDD {
			agncMaxDD = agncDD
		}
		...
		dailyPoints = append(dailyPoints, DailyPoint{
			...
			VooEquity:      vooEquity,
			VooDD:          vooDD,
			AgncEquity:     agncOnlyEquity,
			AgncDD:         agncDD,
			AgncShares:     agncShares,
			UproShares:     uproShares,
			Cash:           cash,
			CumulativeDivs: cumulativeDivs,
	...
	fmt.Printf("\n=====================================================================================================\n")
	fmt.Printf("🚀 3-DAY DROP / 8-DAY HOLD / +5%% TP (65%% ALLOC) + AGNC HIGH-YIELD DIVIDEND SWEEP\n")
	fmt.Printf("=====================================================================================================\n\n")

	table := tablewriter.NewWriter(os.Stdout)
	table.SetHeader([]string{"Performance Metric", "65% UPRO Dip + AGNC Sweep", "AGNC Buy & Hold (With Divs)", "VOO Buy & Hold Benchmark"})
	...
	table.Append([]string{"Ending Capital", fmt.Sprintf("💰 $%.2f", stratFinal), fmt.Sprintf("$%.2f", agncFinal), fmt.Sprintf("$%.2f", vooFinal)})
	table.Append([]string{"Total Net Profit", fmt.Sprintf("+$%.2f", stratProfit), fmt.Sprintf("+$%.2f", agncProfit), fmt.Sprintf("+$%.2f", vooProfit)})
	table.Append([]string{"Total Return (%)", fmt.Sprintf("+%.2f%%", (stratFinal-*initialCapital) / *initialCapital * 100.0), fmt.Sprintf("%+0.2f%%", (agncFinal-*initialCapital) / *initialCapital * 100.0), fmt.Sprintf("+%.2f%%", (vooFinal-*initialCapital) / *initialCapital * 100.0)})
	table.Append([]string{"CAGR (Annualized)", fmt.Sprintf("🔥 %.2f%% / yr", stratCAGR), fmt.Sprintf("%.2f%% / yr", agncCAGR), fmt.Sprintf("%.2f%% / yr", vooCAGR)})
	table.Append([]string{"Max MTM Drawdown", fmt.Sprintf("🔴 %.2f%%", stratMaxDD), fmt.Sprintf("🔴 %.2f%%", agncMaxDD), fmt.Sprintf("🔴 %.2f%%", vooMaxDD)})
	table.Append([]string{"Total Dividends Collected", fmt.Sprintf("💵 +$%.2f (60 Payouts)", cumulativeDivs), fmt.Sprintf("💵 +$%.2f (60 Payouts)", agncOnlyDivs), "$0.00 (Price Only)"})
	...
	generateAgncHTMLReport(*htmlOutput, dailyPoints, trades, stratFinal, vooFinal, agncFinal, stratProfit, vooProfit, agncProfit, stratMaxDD, vooMaxDD, agncMaxDD, cumulativeDivs, winRate, pf)
	fmt.Printf("\n✨ Interactive AGNC dividend sweep chart saved to: %s\n\n", *htmlOutput)
}

func generateAgncHTMLReport(
	outputPath string,
	dailySeries []DailyPoint,
	trades []ExecutedTrade,
	stratEnd, vooEnd, agncEnd, stratPnl, vooPnl, agncPnl, stratDD, vooDD, agncDD, divs, winRate, pf float64,
) {
...
<head>
    ...
    <title>UPRO 65% Dip Strategy + AGNC High-Yield Dividend Sweep</title>
...
<body>
    <div class="container">
        <div class="header">
            <div>
                <h1 class="title">🚀 65% UPRO Dip Strategy + AGNC High-Yield Dividend Sweep</h1>
                <p class="subtitle">Holding Idle Reserves in AGNC (~14% Yield) with Ex-Dividend Date Payout Automation ($100k Base)</p>
            </div>
            <div class="controls">
        ...
        <div class="kpi-grid">
            ...
            <div class="kpi-card">
                <div class="kpi-label">Dividends Collected</div>
                <div class="kpi-value" style="color: var(--agnc);">${{DIVS_COLLECTED}}</div>
                <div class="kpi-sub">60 Monthly AGNC Ex-Div Payouts</div>
            </div>
            <div class="kpi-card">
                <div class="kpi-label">Strategy Max Drawdown</div>
                <div class="kpi-value" style="color: #38bdf8;">{{STRAT_DD}}%</div>
                <div class="kpi-sub">Affected by 2022 Mortgage REIT Decline</div>
            </div>
            <div class="kpi-card">
        ...
        <div class="chart-card">
            <div class="chart-header">
                <div class="chart-title">
                    ...
                    <span class="legend-tag"><span class="dot" style="background: #10b981;"></span><strong>Strategy (65% UPRO Dip + AGNC Sweep)</strong></span>
                    <span class="legend-tag"><span class="dot" style="background: #a855f7;"></span><strong>VOO Buy & Hold</strong></span>
                    <span class="legend-tag"><span class="dot" style="background: #f59e0b;"></span><strong>AGNC Buy & Hold (With Reinvested Divs)</strong></span>
                </div>
            </div>
        ...
        function buildEquityData(pts) {
            return {
                ...
                datasets: [
                    ...
                    {
                        label: 'AGNC Buy & Hold (with Divs)',
                        data: pts.map(p => p.agnc_equity),
                        borderColor: '#f59e0b',
                        borderWidth: 1.5,
                        borderDash: [2, 2],
                        pointRadius: 0,
	...
	replacements := map[string]string{
		...
		"{{AGNC_END}}":        fmt.Sprintf("%0.2f", agncEnd),
		"{{STRAT_PNL}}":       fmt.Sprintf("%0.2f", stratPnl),
		"{{VOO_PNL}}":         fmt.Sprintf("%0.2f", vooPnl),
		"{{AGNC_PNL}}":        fmt.Sprintf("%0.2f", agncPnl),
		"{{STRAT_DD}}":        fmt.Sprintf("%0.2f", stratDD),
		"{{VOO_DD}}":          fmt.Sprintf("%0.2f", vooDD),
		"{{AGNC_DD}}":         fmt.Sprintf("%0.2f", agncDD),
		"{{DIVS_COLLECTED}}":  fmt.Sprintf("%0.2f", divs),
